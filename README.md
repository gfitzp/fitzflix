...
The Queue page shows more than the raw task tables: every file moving through the pipeline leaves an ordered **trail** — Localizing → Moving into the library → Cataloging → Archiving to S3, plus remuxes, transcodes, and restores — rendered as per-stage status chips (green done, blue running, gray queued, amber waiting-to-retry, red failed) and refreshed every five seconds. Trails come from job-lifecycle hooks around the queues and workers, so they track deferred retries and failures without any task instrumentation, and linger for three days.
