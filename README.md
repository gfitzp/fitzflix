...
Each user also has a **watchlist**: any movie page (owned or not) has an add/remove toggle, the My Watchlist page shows everything as a poster gallery with streaming availability, Radarr request buttons, and per-film removal, and logging a film — by hand, from Plex, or via a Letterboxd import — removes it automatically. Watchlisted films influence the recommendation shelves (below) without blocking them, and feed the taste profile as a mild interest signal.

The **Recommendations** page is deep discovery beyond the landing shelves: ten shelves of taste-ranked picks whose kinds shuffle down the page daily — genre and decade shelves, "liked by people who liked…" co-preference shelves whose paired films always share a genre, and occasional shelves fronted by a single anchor film and filled with its nearest neighbors. On a phone each shelf holds its anchor card fixed beside a one-card swipe strip. It replaced the earlier Rate Films drive; seeding taste data now happens in place — every unrated film's tile and hover card carries the estimate-previewing rating ladder, so ratings accumulate wherever you browse, with **Add to watchlist** and **No Opinion** (for films seen but unremembered as much as never seen) always at hand.

Fitzflix is gallery-first: the landing shelves and rails, the movie library, filmographies, local search's movie results, the watchlist, the Criterion spine catalog, the leaving-Criterion pages, and the suggestion strips all show films as poster walls. Every poster carries a **hover card**: hovering (or tapping, on a phone) pops a compact card with the film's credits, synopsis, availability badges, the live rating ladder (your verdict, or the engine's estimate until you have one), and a watchlist toggle. On a phone the poster tap only toggles the card — tap anywhere else to close it, and open the film's page through the card's title. Everything on the card acts in place without leaving the page, and rating or banking a film from a card never disturbs the rating drive's "Since you liked…" strip — only rating the featured film (or a film on its own page) steers the drive.
...
The landing page is built around "what should we watch tonight". The top shelf is **your watchlist, tonight**: only watchlisted films you can actually watch — owned, or streaming on your services — with the month's leaving-Criterion departures leading and everything else in your pinned order. Every rail below is pure discovery and excludes the watchlist entirely: a **library shelf** of owned films picked from a taste-ranked pool so that nothing repeats within roughly a month, a **Watch it again** shelf of old favorites not seen in two years or more, a **streaming shelf** of films on the services you've picked (see below), **Newly added** shelves of recent arrivals on your services' catalogs (Criterion Channel today; the machinery is per-provider, with a full arrival inventory at `/newly-added`), and — for Criterion Channel subscribers — an **On Criterion24/7 now** card showing what the Channel's 24/7 feed is airing this minute (scraped from [whatsonnow.criterionchannel.com](https://whatsonnow.criterionchannel.com) by a poller that re-checks right as each film ends; the card carries the TMDB poster and rating ladder on a director-verified match, filmography-linked credits, and Watch Live/More links) plus a **Leaving the Criterion Channel** shelf of the month's departures with a full inventory page behind it.

The shelves **freeze for the calendar day** — the same films hold the same slots all day, a film that stops being eligible mid-day (watched, waved off) is replaced in its slot without reshuffling the rest, and the next day deals fresh — while the runtime filter ("only films that fit your evening") re-deals every shelf live. Each card says *why* it was picked. Poster corner **folds** carry status site-wide, Plex-style: a green fold for a film newly arrived on your services (or newly in the library), a red fold for one leaving the Criterion Channel soon — one fold per poster with leaving outranking arriving, glyphs alongside color, and owned films never wear the leaving fold (nothing is leaving *you*).
...
- **Catalog discovery** — the recommendation universe reaches past the library: films in your flat-rate services' catalogs are swept into records automatically when the engine's estimate for you clears 3.0 stars, so the streaming and newly-added shelves can recommend films Fitzflix had never otherwise met.
...
Watchlisted films can also **alert you**: opt in on the Profile page and the nightly availability check emails one batched digest per user — never per-film mail — covering watchlisted films that became newly available (a first local file, or newly streaming on a subscribed flat-rate service; rentable-only counts as available under a separate opt-in, since a rental is an extra fee) and films about to leave the Criterion Channel, each entry led by its poster. "Recently available" spans the last month, and a local file alerts only on the film's first arrival, never on quality upgrades.
...
The **TV library** page carries its own search, split into series and episode sections — episode results match on the episode titles carried in filenames, so "the one with the title you remember" is findable without knowing the show's season numbering.
...
The library doubles as a guessing game. A nightly task pre-extracts a pool of frames from the library's films (black and near-empty frames are filtered out; the pool is served through an authenticated route so a frame's filename can't hint at its answer), and the **Name That Frame** page deals them one at a time, tracking per-difficulty win rates with a resettable score. Multiple-choice rounds draw **cast-aware distractors** — wrong answers that share actors with the right one, so the faces don't give it away. **Difficult** switches to free-text guessing with forgiving matching: punctuation squeezed out, spelled-out numbers folded to digits, and a subtitle alone or a pre-subtitle title accepted. **Extra Difficult** shows only a small roaming crop of the frame, confined to the picture's active area — a round can trade its guess for one wider zoom-out, and past that first zoom it can only be won or surrendered. Easy mode deals only films you've rated, a filter keeps any mode to films you've seen, and a zoomed win earns a shareable brag image.
...
For series Sonarr manages, Sonarr is also the **episode-data authority**: Fitzflix matches its series to Sonarr's by library folder (guarded by file coverage, so a coincidental folder-name match can't claim a show) and takes matched series' episode titles from Sonarr's TVDB records. This exists for the shows TMDB numbers differently than the files do — daily and panel shows especially — where TVDB's ordering is the one the episode files follow.
...
Every user can set a **playback device** — the Apple TV their films should be sent to. With one set, a **▶ Play on Apple TV** button appears for them on every owned film — on the film's own page and inside the poster popover card, so a film can be sent to their screen from any poster gallery, and each household member's button targets their own TV. Playback can go through **Plex or [Infuse](https://firecore.com/infuse)** — each user picks a default player on their Profile page. Infuse playback opens the film by its TMDB id over the Apple TV's Companion protocol after a one-time on-screen PIN pairing driven from the Profile page; file pages recommend Infuse for files whose formats outrun Plex's own player, such as Dolby Vision profile 8 with DD+ Atmos audio. Fitzflix builds a play queue on the Plex server and hands it to the player over the [Plex Companion protocol](https://support.plex.tv/articles/202485658-plex-companion-getting-started/); the Plex app must be **open on the device** (tvOS can't wake a backgrounded app — if the TV isn't on Plex, the button reports "is the Plex app open?" rather than playing).
...
The library can broadcast itself: Fitzflix emulates a network tuner that Plex's Live TV & DVR treats like cable — a dial of 24/7 channels with a full guide grid, streamed entirely from files you own. The default dial (seeded on the first lineup build) is an all-library mix, the library's deepest movie genres, a **Criterion** channel of owned films currently streaming on the Criterion Channel, a **Leaving Soon** last-call marathon of owned films in the month's leaving set (each guide entry leads with the departure date), the deepest TV genres, and themed channels like **Game Shows** and **British Sitcoms**.

From there the dial is data, edited at **Admin → DVR Channels**: create, delete, renumber, or disable channels; match members by rules — genres, TMDB keywords, network origin country, title pins (substring matches that pull a title in past every other rule), the Criterion/leaving overlays, movies and/or TV — and pin explicit films or series by title, with lookahead suggestions as you type. A channel can mix both libraries: series air as interleaved broadcast-order blocks like syndication, each series' share proportional to its depth and its cursor marching a few episodes forward per day, while films space themselves through the episode cycle like a nightly feature. Lineups rebuild nightly (rotating like the landing shelves) and after every edit; Plex shows changes at its next guide refresh.

The scheduling is deterministic and stateless: a channel is a stored program list plus an epoch timestamp, and the guide and the stream both compute "what's on right now" from the same arithmetic over real container durations (ffprobed once per file and cached), so they can never disagree. Nothing runs while nobody's watching — when Plex tunes a channel, ffmpeg joins the file mid-program at the schedule's offset and transcodes to H.264 + AC-3 MPEG-TS in real time (VideoToolbox-accelerated on macOS), so the cost is one hardware transcode per tuned channel and zero at idle. A channel play never lands in anyone's diary (see [Tracking Plex watches](#tracking-plex-watches)).

Setup (Plex Pass required for Live TV & DVR):

1. Set `DVR_TOKEN` in `.env` to a long random string (e.g. `python3 -c "import secrets; print(secrets.token_hex(16))"`) and restart — the token gates every DVR endpoint, which answer 404 to anything else. When Plex runs on a different machine than Fitzflix, also set `DVR_TUNER_URL` to the address *Plex* reaches Fitzflix at (default `http://127.0.0.1:8000` — the loopback default matters when a CDN fronts the public hostname, since live streams should never route through it).
2. The **DVR Channels** admin page shows the two URLs to paste, with copy buttons. The tuner address goes into Plex's **Settings → Live TV & DVR → Set Up** under *"Don't see your HDHomeRun device? Enter its network address manually"* — Plex has no native M3U support; Fitzflix answers the HDHomeRun HTTP protocol at that address, which manual entry accepts without any network discovery. The XMLTV guide URL goes in at the channel-setup step (**Edit** under Country → **XMLTV**). If the tuner appears but won't add on the first pass, close the setup dialog and run Set Up again — a long-standing Plex quirk.
3. Map channels if asked (the tuner's channel numbers and names match the guide's) and let the guide download.

Sizing knobs, all optional: `DVR_GENRE_CHANNELS` and `DVR_TV_CHANNELS` (how many movie/TV genre channels the first build seeds), `DVR_CHANNEL_FILMS` and `DVR_CHANNEL_EPISODES` (programs per lineup), and `DVR_VIDEO_BITRATE_KBPS` (stream quality, default 8000).
...
| `DVR_TOKEN`, `DVR_TUNER_URL` | Virtual DVR channels: the token gates the tuner, guide, and stream endpoints (unset disables the feature entirely), and the tuner URL is the Fitzflix origin as Plex reaches it (see [Virtual DVR](#virtual-dvr-live-tv-channels-in-plex)) |
...
Transcoded copies are tracked as **derived files**: every Handbrake output gets a database record linked to its library original (`flask transcodes adopt` sweeps up any untracked copies already on the transcoded tree), the file's page lists its copies, and deleting or replacing an original removes its derived copies with it — rows and physical files both. Derived files live outside the File table by design, so they can never appear in quality rankings, the shopping lists, or the import's replace logic.

Every file moving through the pipeline leaves an ordered **trail** — Localizing → Moving into the library → Cataloging → Archiving to S3, plus remuxes, transcodes, and restores — shown on the **Pipeline Activity** page (linked from Library Maintenance) as per-stage status chips (green done, blue running, gray queued, amber waiting-to-retry, red failed), refreshed every five seconds. Trails come from job-lifecycle hooks around the queues and workers, so they track deferred retries and failures without any task instrumentation, and linger for three days.
...
The **Library Maintenance → Lossy Files** report tracks files whose audio could be better (files carrying the TrueHD Atmos supplement trio are recognized as already-settled and excluded). For files where which track *should* lead is a judgment call, the triage flow generates **listening-clip comparisons**: six short clips per comparison, drawn from five points across each track's full runtime, playable side by side in the browser — then one click either remuxes the file with the chosen track in the lead or dismisses it as fine as-is.

Two more review queues ride alongside: a **TMDB triage** page for records whose metadata match deserves a second look, and a report of files whose measured duration disagrees with their film's TMDB runtime (a wrong match, a partial rip, or an alternate cut wearing the wrong label).
