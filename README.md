...
Radarr and Fitzflix share the movie folders. A TMDB refresh can rename a folder, for example after a change of the title or the year. Fitzflix then points the Radarr movie at the new folder and asks Radarr for a rescan. Without this step, Radarr sees no file and downloads the film again. If a refresh moves a record to a different TMDB id, Fitzflix removes the Radarr entry of the old id. The files stay on the disk. A Radarr failure does not stop the refresh. Fitzflix writes a log line instead.
