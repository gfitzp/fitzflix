...
def start_pairing(user_id, address):
    ...
    its PIN within a few seconds of the task starting."""
...
def pair_task(user_id, address):
    ...
    with app.app_context():
        asyncio.run(_pair(app, user_id, address))


async def _pair(app, user_id, address):
    ...
    try:
        ...
        report(
            "error:Pairing failed — check the address and PIN, make sure "
            "the Apple TV is awake, and try again."
        )
    finally:
